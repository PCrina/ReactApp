...
function App() {
  ...
  const [loacationStored, setLocationStored] = useLocalStorageState(
    [],
    "location"
  );
  ...
  useEffect(
    ...
    [location, setLocationStored]
  );

  return (
    <div className="app">
      <h1>Classy Weather</h1>
      <Input locationInput={location} onSetLocation={setLocation} />
      ...
      <p>Data from localStorage: {loacationStored}</p>
    </div>
  );
}

function Input({ locationInput, onSetLocation }) {
  return (
    <div>
      <input
        type="text"
